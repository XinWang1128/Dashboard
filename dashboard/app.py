        ...
        def lu_map():
            ...
            geo_layer = IpylGeoJSON(
                ...
                style={
                    ...
                    "fillColor": "#f3f3f3",    # Fill color - green
            ...
            for feature in geojson_data['features']:
                ...
                # 1) Normalise to a list of polygons
                #    Polygon:    [ring1, ring2, ...]
                #    MultiPolygon: [ [ring1, ring2, ...], [ring1, ...], ... ]
                if gtype == "Polygon":
                    polygons = [coords]          # wrap single polygon in list
                elif gtype == "MultiPolygon":
                    polygons = coords            # already list of polygons
                else:
                    # skip non-polygon geometries
                    continue

                for poly in polygons:
                    # 2) outer ring = first ring
                    outer_ring = poly[0]         # [[lon, lat, (alt?)], ...]

                    # 3) Convert [lon, lat, ...] -> (lat, lon) safely
                    locations = []
                    for pt in outer_ring:
                        # pt can be [lon, lat] or [lon, lat, alt]
                        lon = pt[0]
                        lat = pt[1]
                        locations.append((lat, lon))   # flip order for ipyleaflet

                    # 4) Create polygon on the map
